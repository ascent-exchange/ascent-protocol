...
contract AirdropClaim is ReentrancyGuard {
    ...
    // 60% veAEX, 40% 4 weeks linear $AEX
    uint256 constant public PRECISION = 1000;
    uint256 constant public VESTED_SHARE = 600;
    uint256 constant public INSTANT_DISTRO = 0;
    uint256 constant public LINEAR_DISTRO = 400;

    uint256 public startTimestamp;

    uint256 public constant DISTRIBUTION_PERIOD = 4 * 7 * 86400;
    ...
    /// @notice set user infromation for aex claim
    /// @param _who is claiming
    /// @param _to who's getting aex token
    ...
    function setUserInfo(address _who, address _to, uint256 _amount) external onlyMerkle nonReentrant returns(bool status) {
        ...
        uint256 _aexInstantAmount = _amount * INSTANT_DISTRO / PRECISION;
        uint256 _aexLockedLinearAmount = _amount * LINEAR_DISTRO / PRECISION;
        uint256 _tokenPerSec = _aexLockedLinearAmount * PRECISION / DISTRIBUTION_PERIOD;

        UserInfo memory _user = UserInfo({
            totalAmount: _amount,
            initAmount: _aexInstantAmount,
            vestedAmount: _vestedAmount,
            lockedAmount: _aexLockedLinearAmount,
            tokenPerSec: _tokenPerSec,
            lastTimestamp: startTimestamp,
            claimed: _aexInstantAmount + _vestedAmount,
            to: _to
        });
        ...
        token.safeTransfer(_to, _aexInstantAmount);
        token.approve(ve, 0);
    ...
    function claim() external nonReentrant {
        ...
        // check if timestamp is > than aex vesting period timestamp
